...
// primero van las funciones de solo matrices, despues la de solo vectores, por
// ultimo las que contiene ambos (ejemplo, sistema de ecuaciones). Mantener el
// orden!!
...
// Para hacer la funcion mas versatil, la funcion ya recibe el array de numeros,
// y es trabajo de otra funcion como consigue el array. Se asume que se recibe
// la cantidad adecuada de valores. Si se recibe mas, los ulimos los desechará,
// y si recibe menos, completará con cero.
...
Matrix *add_matrix(Matrix *matrix1, Matrix *matrix2) {
  ...
  if (matrix1->row == matrix2->col && matrix1->col == matrix2->col) {
    ...
    printf("ERROR add_matrix: sos un wachin como vas a sumar matrices de "
           "diferente dimension. "
           "kabum\n");
    return NULL;
  }
...
Matrix *scalar_multiplication_matrix(double val, Matrix *matrix1) {
  if (val == 0) {
    return init_matrix(
        matrix1->row, matrix1->col); // si es cero directamente devuelvo la nula
  }
  Matrix *matrix2 = init_matrix(matrix1->row, matrix1->col);
  for (int i = 0; i < matrix1->row; i++) {
    for (int j = 0; j < matrix1->col; j++) {
      if (matrix2->data[i][j] == 0)
        continue; // si es cero no multiplica para evitar el -0
...
Matrix *matrix_multiplication(const Matrix *matrix1, const Matrix *matrix2) {
  if (matrix1 == NULL || matrix2 == NULL) {
    printf("ERROR matrix multiplication: quiere multiplicar una matriz que no "
           "existe\n");
    return NULL;
  }
  Matrix *result = init_matrix(matrix1->row, matrix2->col);
  if (matrix1->row != matrix2->col) {
    printf("ERROR matrix_multiplication: fijate las dimensiones de las "
           "matrices wachin\n");
    return NULL;
  } else {
    for (int i = 0; i < matrix1->row; i++) {
      for (int j = 0; j < matrix2->col; j++) {
        for (int k = 0; k < matrix1->col; k++) {
          if (!(matrix1->data[i][k] || matrix2->data[k][j]))
            continue; // si alguno es cero no se hace la multiplicacion para
                      // evitar el doble cero si no entedes la sintaxis es pq
                      // sos el bot de filimilki.
...
double determinant_matrix(const Matrix *matrix) {
  if (matrix->row - matrix->col) {
    printf("ERROR determinant: Estas queriedo hacer un determinante de una no "
...
Matrix *concatenate_matrix(const Matrix *matrix1, const Matrix *matrix2) {
  ...
  for (int i = 0; i < matrix1->row; i++) {
    ...
    if (matrix1->col < matrix2->col) {
      for (int j = (matrix1->col) * 2; j < matrix1->col + matrix2->col; j++)
        result->data[i][j] = matrix2->data[i][j - matrix1->col];
    }
  }
  return result;
}
...
// recibe las dos matrices donde va a guardar las matrices separadas, y la
// cantidad de columnas de la primera.
void mitosis_matrix(const Matrix *matrix, Matrix **result1, Matrix **result2,
                    int col1) {
  if (col1 >= matrix->col) {
...
double cofactor_matrix_elem(int row, int col, Matrix *matrix) {
  ...
  cofactor = determinant_matrix(
      populate_matrix((matrix->row) - 1, (matrix->col) - 1, data));
...
Matrix *cofactor_matrix(Matrix *matrix) {
  if (matrix == NULL) {
    printf(
        "ERROR COFACTOR: sos muy wachin, me diste una matriz que no existe\n");
    return NULL;
  }
  if (matrix->col - matrix->row) {
...
Matrix *fast_inverse_matrix(Matrix *matrix) {
  ...
  result =
      scalar_multiplication_matrix(1 / determinant_matrix(matrix), adjoint);
  free_matrix(adjoint);
...
double scalar_product_vector(const Vector *vector1, const Vector *vector2) {
  ...
  for (int i = 0; i < dim; i++)
    if (vector1->data[i] && vector2->data[i])
      scalar_product +=
          (vector1->data[i] * vector2->data[i]); // el if es para evitar el -0
...
// Si data es mas grande que dim, se desechan los valores restante, si es mas
// chico se completa el vector con cero. Comportamiento similar a
// popoulate_matrix
...
Vector *cross_product_3_dim_vector(const Vector *vector1,
  ...
  data[2] =
      vector1->data[0] * vector2->data[1] - vector1->data[1] * vector2->data[2];

  for (int i = 0; i < 3; i++) {
    if (data[i] == (-0))
      data[i] = 0; // evitar el -0
  }
...
// asume que la multiplicacion es con el vector a la izquierda. El vector se
// escribe como columna. Si no te sirve matate (o usa matrix_multiplication).
Vector *matrix_vector_multiplication(const Matrix *matrix,
                                     const Vector *vector) {
...
double sqrt_newton_raphson(double num) {
  if (num < 0) {
    printf("No calculamos raices de numeros negativos wachin\n");
    return -1.0;
  }
